...
const CodeBlock = ({ code, language }) => {
  ...
  return (
    <div className="relative group">
      <button
        onClick={handleCopy}
        className="absolute top-2 right-2 bg-slate-700 hover:bg-slate-600 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center gap-1 print:hidden"
...
`
  ...
  const parsedContent = useMemo(() => {
    try {
      ...
      // Extract code blocks for React components
      const codeBlocks = [];
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      if (validExtensions.includes(fileExtension)) {
        ...
        setError(`Please upload a valid markdown file (${validExtensions.join(', ')})`);
      }
    }
  };
  ...
  // Render preview with React components for code blocks
  const renderPreview = () => {
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = parsedContent.html;
    
    const pres = tempDiv.querySelectorAll('pre');
    pres.forEach((pre, index) => {
      const codeBlock = parsedContent.codeBlocks[index];
      if (codeBlock) {
  ...
  return (
    <>
      <style>
        {`
          /* GitHub Markdown CSS - Simplified */
          .markdown-body {
            -ms-text-size-adjust: 100%;
            -webkit-text-size-adjust: 100%;
            margin: 0;
          ...
          .hljs-number, .hljs-string, .hljs-meta .hljs-string,
          .hljs-literal, .hljs-doctag, .hljs-regexp { color: #98c379; }
          ...
          .hljs-meta, .hljs-link { color: #61aeee; }
          .hljs-built_in, .hljs-deletion { color: #e6c07b; }
      ...
      <div className="bg-slate-100 min-h-screen font-sans flex flex-col">
        {/* Fixed Header */}
        <header className="bg-white/90 backdrop-blur-sm border-b border-slate-200 p-3 flex justify-between items-center fixed top-0 left-0 right-0 z-20 print:hidden">
          <div className="flex items-center gap-3">
            <Icon name="fileText" className="text-indigo-600 w-6 h-6" />
            <h1 className="text-lg font-bold text-slate-800 tracking-tight">
              Markdown to PDF
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 text-sm bg-slate-200/80 hover:bg-slate-300 text-slate-700 font-semibold py-2 px-3 rounded-lg border border-slate-300 transition-all duration-200"
            >
              <Icon name="upload" className="w-4 h-4" />
              Upload
            </button>
        ...
        {/* Error Display */}
        {error && (
          <div className="fixed top-16 left-1/2 transform -translate-x-1/2 z-30 bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-lg shadow-lg max-w-md print:hidden">
            <p className="text-sm font-medium">{error}</p>
          </div>
        )}

        {/* Main Content Area */}
        <main className="flex-grow pt-20 flex flex-wrap justify-center gap-4 p-4 print:p-0 print:pt-0">
          {/* Editor Panel */}
          <div className="flex flex-col bg-white rounded-lg border border-slate-200 shadow-md print:hidden" style={{ width: '210mm', minHeight: '297mm' }}>
            <div className="p-3 border-b border-slate-200">
              <h2 className="font-semibold text-slate-700">Markdown Editor</h2>
            </div>
            <textarea
          ...
          {/* Preview Panel */}
          <div className="flex flex-col bg-white rounded-lg border border-slate-200 shadow-md print:border-0 print:shadow-none" style={{ width: '210mm', minHeight: '297mm' }}>
            <div className="p-3 border-b border-slate-200 print:hidden">
              <h2 className="font-semibold text-slate-700">HTML Preview</h2>
            </div>
            <div
              id="print-area"
